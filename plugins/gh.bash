...
function _install_gh() {
	tar="/tmp/gh.tar.gz"
	rm -f "${tar}"
	download_file=$(curl --silent --location https://api.github.com/repos/cli/cli/releases/latest | jq --raw-output '.assets[].browser_download_url | select(endswith("_linux_amd64.tar.gz"))')
