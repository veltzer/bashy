...
## 4. Plugins piping a remote script into a shell - DONE
...
`plugins/rust.sh` is done too. `sh.rustup.rs` is only a shim that downloads
`rustup-init` and runs it, and rust publishes a sha256 next to that binary, so the
plugin now fetches it through the cache, verifies it, and runs the verified copy.
Nothing is executed before it has been checked. Installing this way produced a
working `rustc 1.97.1` and `cargo 1.97.1`.

Two things had to be sorted out for that. `bashy_verify_sha256` could not read
rust's checksum line, `<digest> *./rustup-init`, because it matched on a bare
filename and this one carries a binary marker and a path; it now strips both, with a
test for the format. And the verified copy has to keep the name `rustup-init`, since
the binary reads its own filename and treats an unknown one as a request to proxy
that tool.

Still piping, all into a **non root** shell:

- `ai_claude.sh`, `ai_copilot.sh`, `google_cloud_sdk.sh`

Checked again rather than assumed: none of the three publishes a checksum for what
it serves. The copilot one is a file on a `main` branch, so its content can change
under the same url at any time. There is nothing to verify against, so these stay as
they are, per the reasoning below.
