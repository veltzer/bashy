...
<%text>### Installing from a working copy</%text>
...
to run. The cache lives under `<%text>${XDG_CACHE_HOME:-~/.cache}</%text>/bashy/completions` and is
...
<%text>### Writing an installer</%text>
...
Downloads are cached under `<%text>${XDG_CACHE_HOME:-~/.cache}</%text>/bashy/downloads`, overridable
...
<%text>## Core module load order</%text>
